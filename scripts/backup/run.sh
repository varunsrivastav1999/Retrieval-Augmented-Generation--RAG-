...
backup_postgres() {
  ...
  PGPASSWORD="${PGPASSWORD}" pg_dump \
    ...
    -v 2>> "${BACKUP_DIR}/logs/pg_dump.log"
...
cleanup_old_backups() {
  ...
  local pg_count=$(find "${BACKUP_DIR}/postgres" -name "rag_pg_*.dump" -type f -mtime +${RETENTION_DAYS} | wc -l)
  find "${BACKUP_DIR}/postgres" -name "rag_pg_*.dump" -type f -mtime +${RETENTION_DAYS} -delete
