...
def build_strict_grounding_prompt(
    ...
    if broad_query:
        return (
            ...
            "2. DEEP SYNTHESIS: Provide a comprehensive, detailed, and logically structured response. Connect scattered facts and underlying themes into a cohesive summary.\n"
            "3. INLINE CITATIONS: You MUST include inline citations for every major claim using the format `[Source: <filename>, Page <X>]`. The source filename is provided at the top of each text chunk.\n"
    ...
    return (
        ...
        "RULES & ADVANCED DIRECTIVES:\n"
        "1. ZERO HALLUCINATION POLICY: If the exact answer is not explicitly stated or logically deducible from the provided text, state: 'This information is not available in the uploaded documents.' Never invent, guess, or use external knowledge.\n"
        "2. COMPREHENSIVE ANSWER: Provide a complete and detailed answer. Do not truncate or be overly brief. If the answer is implied by the context, state it confidently with explanation.\n"
        "3. INLINE CITATIONS: You MUST include inline citations for every major claim using the format `[Source: <filename>, Page <X>]`. The source filename is provided at the top of each text chunk.\n"
        "4. TABLE LOOKUP MASTERY & FORMATTING:\n"
        ...
        "5. MULTI-HOP REASONING & COT: If the query requires combining piece A and piece B from different chunks, briefly explain your step-by-step reasoning (Chain of Thought) before concluding the final answer.\n"
        ...
        "8. EXHAUSTIVE EXTRACTION & NO TRUNCATION: Do NOT summarize away important details. Return all related details, clauses, headings, and context exactly as it appears in the documents.\n"
        "9. NO FILLER WORDS: DO NOT use introductory filler phrases like 'According to the provided text'. Start your answer immediately with the requested facts.\n"
        f"{topic_line}\n"
        "-----------------------------------------------------------\n"
        f"DATABASE RECORDS:\n{context_text}\n"
        "-----------------------------------------------------------\n\n"
